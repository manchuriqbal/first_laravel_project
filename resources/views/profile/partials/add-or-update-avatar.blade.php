<section>
    <header>
        <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
            User Avatar
        </h2>
        <div>
            @php
                $avatarUrl = $user->avater;
                if (!filter_var($avatarUrl, FILTER_VALIDATE_URL)) {
                    // If the avatar URL is not a valid external URL, assume it's stored locally
                    $avatarUrl = asset('storage/avatars/' . $avatarUrl);
                }
            @endphp
            <img style="width: 100px; height: 100px; border-radius: 50%;" src="{{ $avatarUrl }}" alt="User Avatar">
        </div>

        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            Add or Update your account's profile avatar.
        </p>
    </header>

    @if (session('message'))
        <div>
            {{ session('message') }}
        </div>
    @endif

    <form method="post" action="{{ route('profile.avatar') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        @method('patch')
        @csrf

        <div>
            <x-input-label for="avatar" value="Avatar" />
            <x-text-input id="avatar" name="avatar" type="file" class="mt-1 block w-full" :value="old('avatar', $user->avatar)" autofocus autocomplete="name" />
            <x-input-error class="mt-2" :messages="$errors->get('avatar')" />
        </div>

        <div class="flex items-center gap-4">
            <x-primary-button>{{ __('Save') }}</x-primary-button>
        </div>
    </form>
</section>
